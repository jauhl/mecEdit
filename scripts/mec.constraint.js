...
mec.constraint = {
    ...
    prototype: {
        ...
        init(model) {
            ...
            this.vel = this.type === 'free' ? () => true
                : this.type === 'rot' ? () => this.len_vel()
                    : this.type === 'tran' ? () => this.ori_vel()
                        : this.type === 'ctrl' ? () => { let res = this.ori_vel(); return this.len_vel() && res }
                            : false;
        ...
        ori_pos() {
            ...
            //            console.log('C-ori('+this.id+')='+(C_x**2 + C_y**2)+' < '+(mec.linTol));
        ...
        len_pos() {
            ...
            //            console.log('C-len('+this.id+')='+(this.ax*C)+' / '+(this.ay*C));
        ...
        len_vel() {
            ...
            //console.log('Ct('+this.id+')='+Ct+' < '+mec.linTol);
        ...
        g2() {
            ...
            return  g2().beg({ x: p1.x, y: p1.y, w, scl: 1, lw: 2, ls: 'orange', fs: '@ls', lc: 'round', sh: this.sh })
                            .stroke({ d: `M50,0 ${r},0`, ls: this.model.constraintColor, lw: lw + 1 })
                            .drw({ d: mec.constraint.arrow[type] })
                        .end()
                        .txt({ str: id || '?', x: xid, y: yid, thal: 'center', tval: 'middle', ls:'white' })
        },
        get isSolid() { return false; },
        get sh() { return this.state & g2.OVER ? [0, 0, 10, 'white'] : this.state & g2.EDIT ? [0, 0, 10, 'yellow'] : false; },
        // get ls() { return this.type === 'ctrl' ? 'red' : 'orange'},
        // editing ...
        hitContour({ x, y, eps }) { return g2.isPntOnLin({ x, y }, this.p1, this.p2, eps) }, // must pass eps from g2.editor, default eps is way too small
        //        drag({dx,dy}) { this.p1.x += dx; this.y += dy }
    },
    arrow: {
