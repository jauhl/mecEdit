...
g2.symbol.tick = g2().p().m({x:0,y:-2.5}).l({x:0,y:2.5}).stroke({lwnosc:true});
...
g2.prototype.ply.prototype = {
    ...
    hitInner({x,y,eps}) { let p={x:x-this.x,y:y-this.y}; return g2.isPntInPly2(p,this,eps) }, // translational only .. at current .. !
    drag({dx,dy}) { this.x += dx; this.y += dy; },
    handles(grp) {
        let p, slf=this;
...
g2.prototype.spline.prototype = g2.mixin({},g2.prototype.ply.prototype,{
    g2() {
        let {pts,closed,x,y,w,ls,lw,fs,sh} = this, bezcmds = g2.catmulRom(pts,closed), 
            istrf = x || y || w, g = g2();

        if (istrf) g.beg({x,y,w});
        g2.applyCommands(g,bezcmds);
        g.drw({ls,lw,fs,sh});
        if (istrf) g.end();
 ...
 */
g2.prototype.label = function label({str,loc,off,fs,font,fs2,border}) {
...
//                 .rec({x:p.x-n*0.8*h/2/Math.SQRT2,y:p.y-h/2/Math.SQRT2,b:n*0.8*h/Math.SQRT2,h:h/Math.SQRT2})
            label.txt({str, x:p.x,y:p.y,
                       ...
                       tval: "middle",
                       fs: fs||'black',
                       font: font||g2.defaultStyle.font })
        }
        return label;
    }
}
