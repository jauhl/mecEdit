...
g2.prototype = {
    ...
    use({
        ...
        if (typeof grp === "string") arguments[0].grp = grp = g2.symbol[grp];
        if (grp && grp !== this) this.addCommand({
            c: "use",
            a: arguments[0]
        });
        return this
    ...
    exe(ctx) {
        ...
        if (handler && handler.init(this)) handler.exe(this.commands);
        return this
    },
    addCommand({
        c: c,
...
g2.getCmdIdx = function (cmds, callbk) {
    for (let i = cmds.length - 1; i >= 0; i--)
        if (callbk(cmds[i], i, cmds)) return i;
    return false
};
...
g2.canvasHdl.prototype = {
    ...
    async exe(commands) {
        for (let cmd of commands) {
            ...
            } else if (cmd.a && "g2" in cmd.a) this.exe(cmd.a.g2().commands)
        }
    },
    view({
        x: x = 0,
    ...
    async img({
        ...
        this.ctx.drawImage(img_, xoff || 0, yoff || 0, dx || img_.width, dy || img_.height, 0, this.isCartesian ? -h : 0, b || img_.width, h || img_.height);
        this.ctx.restore()
    },
    use({
        grp: grp
    ...
    drw({
        ...
        if (ctx.shadowColor !== "rgba(0, 0, 0, 0)" && ctx.fillStyle !== "rgba(0, 0, 0, 0)" && !lsh) {
            ...
            ctx.shadowColor = shc
        } else d ? ctx.stroke(p) : ctx.stroke();
        this.resetStyle(tmp)
    },
    get: {
    ...
    initStyle(style) {
        for (const key in style)
            if (this.get[key] && this.get[key](this.ctx) !== style[key]) this.set[key](this.ctx, style[key])
    },
    setStyle(style) {
        let q, prv = {};
        for (const key in style) {
    ...
    resetStyle(style) {
        for (const key in style) this.set[key](this.ctx, style[key])
    },
    pushStyle(style) {
        let cur = {};
        for (const key in style)
            if (this.get[key]) {
                ...
                if (this.cur[key] !== style[key]) this.set[key](this.ctx, cur[key] = style[key])
    ...
    popStyle() {
        ...
        for (const key in this.cur)
            if (this.get[key] && this.cur[key] !== cur[key]) this.set[key](this.ctx, this.cur[key])
    },
    concatTrf(q, t) {
...
g2.render = function render(fn) {
    function animate(t) {
        if (fn(t)) requestAnimationFrame(animate)
    }
...
g2 = Object.assign(g2, {
    ...
    isPntOnPly({
        ...
        for (var i = 0, n = pts.length; i < (closed ? n : n - 1); i++)
            if (g2.isPntOnLin({
                    x: x,
                    y: y
                }, pts[i], pts[(i + 1) % n], eps)) return true;
        return false
    },
    isPntOnBox({
        x: xp,
    ...
    isPntInPly({
        ...
        for (let n = pts.length, i = 0, pi = pts[i], pj = pts[n - 1]; i < n; pj = pi, pi = pts[++i])
            if ((y > pi.y || y > pj.y) && (y <= pi.y || y <= pj.y) && (x <= pi.x || x <= pj.x) && pi.y !== pj.y && (pi.x === pj.x || x <= pj.x + (y - pj.y) * (pi.x - pj.x) / (pi.y - pj.y))) match++;
        return match % 2 != 0
    },
    isPntInBox({
        x: xp,
...
g2.prototype.cir.prototype = {
    ...
    pointAt(loc) {
        var q = loc + 0 === loc ? [Math.cos(loc * 2 * Math.PI), Math.sin(loc * 2 * Math.PI)] : this._dir[loc || "c"] || [0, 0],
            nx = q[0],
            ny = q[1];
        return {
            x: this.x + nx * this.r,
...
g2.prototype.arc.prototype = {
    ...
    pointAt(loc) {
        var t = loc === "beg" ? 0 : loc === "end" ? 1 : loc === "mid" ? .5 : loc + 0 === loc ? loc : .5,
...
g2.prototype.ply.prototype = {
    ...
    handles(grp) {
        let p, slf = this;
        for (let n = this._itr.len, i = 0; i < n; i++) grp.handle({
...
g2.prototype.spline = function spline({
    pts: pts,
    closed: closed,
    x: x,
    y: y,
...
g2.prototype.label = function label({
    ...
    let idx = g2.getCmdIdx(this.commands, cmd => {
        return cmd.a && "pointAt" in cmd.a
    });
    if (idx !== undefined) {
    ...
    g2() {
        ...
        if (this._refelem) {
            let {
                ...
                border: border,
                fs2: fs2
            } = this, p = this._refelem.a.pointAt(loc), tanlen = p.dx * p.dx || p.dy * p.dy;
            let h = parseInt(font || g2.defaultStyle.font),
                diag, phi, n;
            if (str[0] === "@" && (s = this._refelem.a[str.substr(1)]) !== undefined) str = "" + (Number.isInteger(+s) ? +s : Number(s).toFixed(Math.max(g2.symbol.labelSignificantDigits - Math.log10(s), 0))) + (str.substr(1) === "angle" ? "°" : "");
            n = str.length;
...
g2.prototype.mark = function mark({
    mrk: mrk,
    loc: loc,
    dir: dir,
    fs: fs,
...
g2.prototype.mark.prototype = {
    ...
    g2() {
        let {
            ...
            fs: fs,
            ls: ls
        } = this, elem = this._refelem.a, marks = g2();
        if (Array.isArray(loc))
            for (let l of loc) marks.use(this.markAt(elem, l, mrk, dir, ls, fs));
        else marks.use(this.markAt(elem, loc, mrk, dir, ls, fs));
        return marks
    }
};
/**
...
g2.prototype.skip = function skip(tag) {
    if (this.cmds.length) this.cmds[this.cmds.length - 1].skip = tag;
    return this
};
...
g2.prototype.chart = function chart({
    x: x,
    y: y,
    b: b,
    h: h,
...
g2.prototype.chart.prototype = {
    g2() {
        ...
        if (funcs && funcs.length) {
            for (const func of funcs) this.initFunc(func, this.xmin === undefined, this.xmax === undefined, this.ymin === undefined, this.ymax === undefined)
        }
    ...
    initFunc(fn, setXmin, setXmax, setYmin, setYmax) {
        ...
        if (itr && (setXmin || setXmax || setYmin || setYmax)) {
            ...
            if (fn.color && typeof fn.color === "number") fn.color = this.defaults.colors[fn.color % this.defaults.colors.length]
        }
    },
    ...
    drawXAxis(g) {
        let tick, showgrid = this.xaxis && this.xaxis.grid,
        ...
        for (let i = 0; i < this.xAxis.N; i++) {
            ...
            if (showgrid) g.lin(Object.assign({
                x1: tick.t,
                y1: 0,
                x2: tick.t,
                y2: this.h},
            ...
            if (showlabels && tick.maj) g.txt(Object.assign({
        ...
        if (showorigin && this.xmin <= 0 && this.xmax >= 0) g.lin({
    ...
    drawYAxis(g) {
        let tick, showgrid = this.yaxis && this.yaxis.grid,
        ...
        for (let i = 0; i < this.yAxis.N; i++) {
            ...
            if (i && showgrid) g.lin( Object.assign({
                y1: tick.t,
                x2: this.b,
                x1: 0,
                y2: tick.t},
        ...
        if (showorigin && this.ymin <= 0 && this.ymax >= 0) g.lin({
            x1: 0,
    ...
    drawFunc(g, fn, defaultcolor) {
        ...
        if (itr) {
            ...
            if (fill) plydata.push(this.pntOf({
                x: itr(0).x,
                y: 0
            }));
            for (let i = 0, n = itr.len; i < n; i++) plydata.push(this.pntOf(itr(i)));
            if (fill) plydata.push(this.pntOf({
                x: itr(itr.len - 1).x,
                y: 0
            }));
            if (fn.spline && g.spline) g.spline(args);
            else g.ply(args);
            if (fn.dots) {
                g.beg({
                    fs: "snow"
                });
                for (var i = 0; i < plydata.length; i++) g.cir( Object.assign({
                    r: 2,
                    lw: 1},
                    plydata[i]
                ));
...
g2.color = {
    rgba(color, alpha) {
        ...
        if (color in g2.color.names) color = "#" + g2.color.names[color];
        if (res = /#([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})/.exec(color)) return {
            r: parseInt(res[1], 16),
            g: parseInt(res[2], 16),
            b: parseInt(res[3], 16),
            a: alpha
        };
        if (res = /#([a-fA-F0-9])([a-fA-F0-9])([a-fA-F0-9])/.exec(color)) return {
            r: parseInt(res[1] + res[1], 16),
            g: parseInt(res[2] + res[2], 16),
            b: parseInt(res[3] + res[3], 16),
            a: alpha
        };
        if (res = /rgb\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*\)/.exec(color)) return {
            r: parseInt(res[1]),
            g: parseInt(res[2]),
            b: parseInt(res[3]),
            a: alpha
        };
        if (res = /rgba\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]+(?:\.[0-9]+)?)\s*\)/.exec(color)) return {
            r: parseInt(res[1]),
            g: parseInt(res[2]),
            b: parseInt(res[3]),
            a: alpha !== undefined ? alpha : parseFloat(res[4])
        };
        if (res = /rgb\(\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*\)/.exec(color)) return {
