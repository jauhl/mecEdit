 ...
 * @requires g2.js
 */
"use strict";mec.view={extend(t){if(t.type&&mec.view[t.type]){Object.setPrototypeOf(t,mec.view[t.type]);t.constructor()}return t}};mec.view.vector={constructor(){},validate(t){const i=["vel","acc","force"];if(this.p===undefined)return{mid:"E_ELEM_REF_MISSING",elemtype:"vector",id:this.id,idx:t,reftype:"node",name:"p"};if(!this.model.nodeById(this.p))return{mid:"E_ELEM_INVALID_REF",elemtype:"vector",id:this.id,idx:t,reftype:"node",name:this.p};else this.p=this.model.nodeById(this.p);if(this.value===undefined)return{mid:"E_ALY_REF_MISSING",elemtype:"vector",id:this.id,idx:t,reftype:"node",name:"value",keys:"["+i.join(",")+"]"};return false},init(t,i){this.model=t;this.model.notifyValid(this.validate(i))},dependsOn(t){return this.p===t},preview(){},reset(){},asJSON(){return'{ "type":"'+this.type+'","id":"'+this.id+'","p":"'+this.p.id+'"'+(this.value?',"value":"'+this.value+'"':"")+" }"},get isSolid(){return false},get sh(){return this.state&g2.OVER?[0,0,10,this.model.env.show.hoveredElmColor]:false},get endPoints(){const t=mec.aly[this.value].drwscl;const i=this.p[this.value];const e=Math.hypot(i.y,i.x);const s=!mec.isEps(e,.5)?mec.asympClamp(t*e,25,100):0;return{p1:this.p,p2:{x:this.p.x+i.x/e*s,y:this.p.y+i.y/e*s}}},hitContour({x:t,y:i,eps:e}){const s=this.endPoints;return g2.isPntOnLin({x:t,y:i},s.p1,s.p2,e)},g2(){const t=this.endPoints;return g2().vec({x1:t.p1.x,y1:t.p1.y,x2:t.p2.x,y2:t.p2.y,ls:this.model.env.show[this.value+"VecColor"],lw:1.5,sh:this.sh})}};mec.view.trace={constructor(){this.pts=[]},validate(t){if(this.p===undefined)return{mid:"E_ELEM_REF_MISSING",elemtype:"trace",id:this.id,idx:t,reftype:"node",name:"p"};if(!this.model.nodeById(this.p))return{mid:"E_ELEM_INVALID_REF",elemtype:"trace",id:this.id,idx:t,reftype:"node",name:this.p};else this.p=this.model.nodeById(this.p);return false},init(t,i){this.model=t;if(!this.model.notifyValid(this.validate(i)))return;this.t0=this.t0||0;this.Dt=this.Dt||1;this.mode=this.mode||"dynamic";this.pts.length=0},dependsOn(t){return this.p===t},build(){const t=this.model.timer.t;if(this.mode==="static"||this.mode==="preview"){if(this.t0<=t&&t<=this.t0+this.Dt)this.pts.push({x:this.p.x,y:this.p.y})}else if(this.mode==="dynamic"){if(this.t0<t)this.pts.push({x:this.p.x,y:this.p.y});if(this.t0+this.Dt<t)this.pts.shift()}},preview(){if(this.mode==="preview")this.build()},reset(t){if(t||this.mode!=="preview")this.pts.length=0},post(t){if(this.mode!=="preview")this.build()},asJSON(){return'{ "type":"'+this.type+'","id":"'+this.id+'","p":"'+this.p.id+'"'+(this.Dt?',"Dt":'+this.Dt:"")+(this.stroke&&!(this.stroke==="navy")?',"stroke":"'+this.stroke+'"':"")+(this.fill&&!(this.stroke==="transparent")?',"fill":"'+this.fill+'"':"")+" }"},get isSolid(){return false},get sh(){return this.state&g2.OVER?[0,0,10,this.model.env.show.hoveredElmColor]:false},hitContour({x:t,y:i,eps:e}){return false},g2(){return g2().ply({pts:this.pts,format:"{x,y}",ls:this.stroke||"navy",lw:1.5,fs:this.fill||"transparent",sh:this.sh})}};mec.view.info={constructor(){},init(t){if(typeof this.elem==="string")this.elem=t.elementById(this.elem)},dependsOn(t){return this.elem===t},reset(){},asJSON(){return'{ "type":"'+this.type+'","id":"'+this.id+'","elem":"'+this.elem.id+'"'+(this.value?',"value":"'+this.value+'"':"")+(this.name?',"name":"'+this.name+'"':"")+" }"},get hasInfo(){return this.elem.state===g2.OVER},infoString(){if(this.value in this.elem){const t=this.elem[this.value];const i=mec.aly[this.value];const e=i.type;const s=t=>(t*i.scl).toPrecision(3);return(this.name||i.name||this.value)+": "+(e==="vec"?"{x:"+s(t.x)+",y:"+s(t.y)+"}":s(t))+" "+i.unit}return"?"}};
/**
 * mec.shape (c) 2018 Stefan Goessner
 * @license MIT License
 * @requires mec.core.js
